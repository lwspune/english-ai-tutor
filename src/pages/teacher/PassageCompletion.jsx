    ...
    <div className="min-h-screen bg-slate-50">
      <header className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3">
        <button
          ...
          className="text-slate-500 hover:text-slate-800 text-sm min-h-[44px] flex items-center"
          aria-label="Back to dashboard"
        >
          ← Back
        </button>
        <h1 className="text-base font-semibold text-slate-800">Passage Completion</h1>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        {loading ? (
          ...
          <p className="text-sm text-slate-400">No passages yet.</p>
        ) : totalStudents === 0 ? (
              ...
              <div key={p.id} className="bg-white rounded-2xl border border-slate-200 p-4">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-0.5">{p.grade_level === 'MBA' ? 'MBA' : `Grade ${p.grade_level}`} · {p.difficulty ?? 'Easy'}</p>
                  </div>
                  {allDone ? (
                ...
                {!allDone && (
                  <div>
                    <p className="text-xs text-slate-400 mb-2">Not read yet ({notDone.length}):</p>
