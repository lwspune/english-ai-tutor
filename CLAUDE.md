...
### Milestones — built
Durable log in the `milestones` table; awarded server-side via `award_milestone` RPC (idempotent via `dedupe_key`). Kinds: streak crossings (5/10/20), personal best accuracy + WPM, comprehension ≥80%, first-mastery of a vocabulary word, drill_session_aced (3 distinct stumble words correct in one session). Read into `StudentProgress` via `fetchRecentMilestones` and rendered by `<MilestoneList>`. Celebration cues (`Confetti` + `feedback('celebrate')`) fire at the trigger surface (StudentHome for streaks, SessionReport for everything else). Good milestones: genuine learning achievements only. Bad milestones forbidden: "logged in 3 days in a row", "earned 100 XP". A `Milestones earned` surface on StudentHome (vs StudentProgress only today) is still open — see ROADMAP.md.
...
Deployment commands, environment variables, manual procedures, and production quirks live in [`OPERATIONS.md`](./OPERATIONS.md). Active product priorities live in [`ROADMAP.md`](./ROADMAP.md). Provisional product decisions (each with a watch-signal and revisit trigger) live in [`DECISIONS.md`](./DECISIONS.md). The Monday-AM observation cadence is described in `memory/process_weekly_observation.md`.
...
- `ReadingSession` (`/student/session/:passageId`) — audio recording with animate-pulse red dot + countdown timer. Recording controls live in a fixed-bottom sticky bar (`recording-bar` testid) — always visible regardless of scroll, so students can't miss Start Recording while skimming the passage. Bar contains a persistent "🎙 Recordings may be kept for teacher review" indicator (retention disclosure). Start Recording disabled when per-passage attempt limit (3) OR daily session limit is reached. Start/Stop buttons fire `feedback('tap')` for tactile confirmation.
...
- `VocabHome` (`/student/vocab`) — grades 11/12/MBA only (others see "Vocabulary practice unlocks in grade 11"). Loads total word count + own progress, shows mastered/total card with indigo progress bar + due-today CTA with Start Practice button. Button disabled when no due AND no new available (`canPractice = due > 0 || newAvailable > 0`). Three contextual hints help students who haven't reached mastery yet: an `in-progress-count` line ("N words in progress — practising your way toward mastery") when `seen > mastered`, a `mastery-timeline-hint` ("first masteries usually take 25+ days of consistent reviews") shown only when `seen > 0 AND mastered = 0`, and a `due-soon-hint` (+ N more due within 24h) inside the due card when unmastered words have `next_review_at` in the next 24h. Also shows a small `seen-from-reading` line under the mastered bar when `last_encounter_source = 'reading'` rows exist. Fixed `BottomNav` at bottom.
...
- **Audio Review** (`/teacher/audio-review`) — lists the latest 100 retained reading recordings (newest first). Per row: student name, passage title, scores (accuracy / WPM / phrasing / comprehension), timestamp, status badge if reviewed. Play button calls `retained-audio-url` → plays audio inline via `<Audio>`. Mark-as buttons (Reviewed / Disputed / No action) call `mark_retention_review` RPC and update the local state. Parent consent obtained offline; surfaces a one-line disclosure at the top of the page.
...
- `src/components/MilestoneList.jsx` — renders the "Recent milestones" card on `StudentProgress`. Maps `kind` to icon + label (🔥 streak, 🎯 personal best accuracy, ⚡ personal best wpm, 📘 comprehension aced, ⭐ word mastered, 💪 drill aced). Returns null when empty.
...
- `milestones` (migration 021; kind list extended in migration 026) — durable record of celebration events. Columns: `id uuid pk`, `student_id uuid` (FK profiles, cascade), `kind text` (CHECK in `streak_5|streak_10|streak_20|personal_best_accuracy|personal_best_wpm|comprehension_aced|word_mastered|drill_session_aced`), `achieved_at timestamptz`, `payload jsonb`, and `dedupe_key text` (STORED generated as `kind || ':' || payload->>'dedupe'`). Unique index on `(student_id, dedupe_key)` enforces idempotency: once-ever kinds (streak crossings) use `dedupe = ''`; repeatable kinds key on `session_id` or `word_id`. RLS: SELECT own + teachers via `is_teacher()`; INSERT/UPDATE/DELETE revoked — all writes go through the `award_milestone` RPC (security-definer; server-validates each claim against source data). Backfilled at migration time from existing sessions + progress rows; future awards happen client-side from the celebration triggers in StudentHome / SessionReport / VocabPractice. Read into `StudentProgress` via `fetchRecentMilestones(studentId, limit)` and rendered by `<MilestoneList>`.

- `drill_attempts` (migration 025) — one row per deliberate-practice attempt. Columns: `id uuid pk`, `student_id uuid` (FK profiles, cascade), `session_id uuid` (FK sessions, cascade), `stumble_word text`, `sentence text`, `score numeric`, `was_correct boolean default false`, `attempt_index smallint` CHECK 1–3, `created_at timestamptz`. Indexes on `(student_id, created_at desc)` and `(session_id, stumble_word)`. RLS: SELECT own + teachers via `is_teacher()`; INSERT/UPDATE/DELETE revoked — all writes go through `analyze-drill` (service role).
- `app_settings` — single-row table (`id boolean PK default true`), holds `ai_feedback_enabled boolean`, `class_code text` (random 6-char code set on migration; teacher shares with students for self-registration), `daily_session_limit int` (default 5; teacher adjusts via dashboard stepper, clamped 1–20), `cron_secret text` (auto-generated in migration 013; shared between pg_cron job and `send-reminders` edge function), and two rolling-retention fields added in migration 028: `audio_retention_enabled boolean default true` and `audio_retention_cap int default 100`. The Phase 1 spike fields (`spike_audio_retention`, `spike_audio_retention_count`, `spike_audio_retention_limit`) were dropped in 028 — the spike retention mechanism is generalised into the rolling-100 research retention below.

- `sessions.retention_reviewed_status text` (added 2026-05-12 in migration 028) — workflow status the teacher sets on retained recordings via `/teacher/audio-review`. CHECK constraint: null or `reviewed | disputed | no_action`. Updated only via `mark_retention_review` RPC (teacher-gated).
...
- `claim_retention_slot()` — service-role only; returns `jsonb { retain: bool, evicted_path: text|null }`. If `audio_retention_enabled` is on and `retained_audio_path` count is at `audio_retention_cap`, finds the oldest retained session, clears its `retained_audio_path`, and returns that path so the caller can delete the corresponding audio file from storage. Called by `analyze-reading` immediately before insert. Replaces the spike-specific `try_claim_spike_slot()` (dropped in migration 028).
- `mark_retention_review(p_session_id uuid, p_status text)` — teacher-only (via `is_teacher()`). Sets `sessions.retention_reviewed_status` to one of `reviewed | disputed | no_action` or null. Called by `AudioReview` after the teacher listens to a retained recording.
...
- `retained-audio-url` (v1) — teacher-only signed URL minter for the rolling-100 retained reading recordings. Body: `{ sessionId }`. Validates JWT, looks up the caller's profile, rejects if `role !== 'teacher'`. Looks up `sessions.retained_audio_path`, returns a 10-minute signed URL via `supabase.storage.from('audio').createSignedUrl`. Deployed with `verify_jwt: false` (new key format); authenticates explicitly inside.
...
- `scripts/spike/spike_compare.py` — orchestrator. Reads a manifest, downloads retained audio via the signed-URL edge function (was `spike-audio-url`, now retired; on revival, point at `retained-audio-url` which the rolling-100 mechanism feeds), runs FA, writes `spike-audio/results.csv` + per-session detail JSON.
...
| Rolling-100 audio retention for research | Migration 028 generalised the spike-specific retention into always-on rolling-100 retention. `analyze-reading` calls `claim_retention_slot()` per session; FIFO eviction at cap. Teacher reviews via `/teacher/audio-review` (`retained-audio-url` edge function for signed playback). Persistent in-app indicator on the ReadingSession recording bar. | Parent consent obtained offline; intent is research / analysis / app improvement only. Hard cap in code (100), no quiet drift. Teacher-only access via `is_teacher()` + JWT validation. Mechanism subsumes the prior spike-retention path so FA spike revival reuses the same infrastructure. |
