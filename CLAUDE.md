...
- `award_milestone(p_kind text, p_payload jsonb)` — migration 021. Server-validates each kind against source data (streak via `compute_student_streak`; personal-best / comp-aced against `sessions`; word_mastered against `student_word_progress`). Idempotent via the generated `dedupe_key` column on `milestones`.
