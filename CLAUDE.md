...
- `grade_vocab_attempt(p_word_id uuid, p_was_correct boolean)` — security-definer, granted to `authenticated`. Atomically upserts the caller's `student_word_progress` row applying the Leitner SRS rule (intervals `[1,3,7,14,30]` days, max box 5). Correct increments `correct_count`; wrong drops to box 1 (count preserved). `mastered_at` is set when reaching box 5 with `correct_count >= 3` and stays set thereafter — subsequent attempts still increment `total_encounters` but never un-master.

### Edge functions
- `analyze-reading` — main reading evaluation pipeline (Whisper + GPT-4o-mini); saves cost metrics to sessions. Before deletion, calls `try_claim_spike_slot()`; when a slot is claimed, audio is kept and `spike_audio_path` is written to the session row.
...
- **Spike-only (remove after Phase 1 decision):** `spike-audio-url` accepts `{ sessionId }` and returns a 10-minute signed URL for the matching `spike_audio_path`; `spike-cleanup` deletes every retained audio file from the `audio` bucket and clears `spike_audio_path` on all sessions. Both use `verify_jwt: false` and authorise via service role internally.
