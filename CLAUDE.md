...
- `generate-vocab-audio` — service-role-only helper. Accepts `{ word_ids: string[] }` or `{ limit }` (defaults to 20, max 30) and fetches `audio_path is null` rows itself when no ids passed. For each row: calls OpenAI `tts-1` with voice `nova`, uploads the MP3 to `vocab-audio/pronunciation/{word_id}.mp3`, sets `audio_path`. Returns `{ results, remaining_pending }`. Called by `scripts/vocab/seed_audio.py` in a poll loop; one-shot ~$0.13 to seed all 865 words.
