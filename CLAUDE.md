...
- **Retired (2026-05-12, Phase D of security-review fix plan)** — `vocab-seed-batch` and `generate-vocab-audio` both accepted unauthenticated POST and called OpenAI, creating a cost-attack vector. Replaced with 410-Gone stubs in the repo; deployments updated. The work they served is complete (`entries.json` is the canonical word list; 865/865 `audio_path` rows populated). Restore from git history if vocab is ever expanded.
- **Deployed via MCP but not in repo source** — three other one-shot helper functions live on the project with no `supabase/functions/*` directory:
  ...
  - All three deploy with `verify_jwt: false`. The spike pair authorises via service role internally (and uses obscure session-ids as a practical capability). They're follow-up retirement candidates once the Phase 1 spike resolves.
...
| TTS for stumble words | Generate-on-demand via the existing OpenAI `tts-1` / `nova` pipeline into the public `vocab-audio` bucket at `pronunciation/stumble/{word}.mp3`. | Stumble words can be ANY passage word, so the vocab v2 pre-seed model doesn't fit. Generate-on-demand is cheap (~$0.015/1k chars) and the audio is non-sensitive (public CDN delivery is fine). v1 reuses the same `AudioPlayButton` component with a different path prefix. |
